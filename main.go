...
func main() {
	...
	//multiCoresWithoutIO() //从输出结果看，多核并行（parellel），很难看出效果
	oneCoreWithIO()         //从输出结果看，单核并发（concurrency），后面应该有一个队列，每个协程从队列里依次取出来执行。发生了io请求，当前协程主动挂起，让其他协程运行
	//multiCoresWithIO()    //从输出结果看，多核并行（parellel），很难看出效果
}
